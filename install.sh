...
# Add Sury's PHP repository with better error handling
if [ ! -f "/usr/share/keyrings/php-sury-keyring.gpg" ]; then
    # Install prerequisites
    apt install -y -q lsb-release ca-certificates apt-transport-https gnupg2
    check_exit_code 4.1
    
    # Download and verify the GPG key
    wget -qO /tmp/php-sury.gpg https://packages.sury.org/php/apt.gpg
    if ! gpg --show-keys /tmp/php-sury.gpg >/dev/null 2>&1; then
        echo "ERROR: Invalid GPG key downloaded"
        exit 1
    fi
    gpg --dearmor -o /usr/share/keyrings/php-sury-keyring.gpg < /tmp/php-sury.gpg
    rm /tmp/php-sury.gpg
    check_exit_code 4.2
    
    # Configure the repository with architecture
    echo "deb [arch=$(dpkg --print-architecture) signed-by=/usr/share/keyrings/php-sury-keyring.gpg] https://packages.sury.org/php/ $(lsb_release -sc) main" > /etc/apt/sources.list.d/php.list
    check_exit_code 4.3
...
echo_task 5/40 "Install PHP 8.3 and necessary extensions"
...
echo_task 6/40 "Create backup directories"
...
echo_task 11/40 "Remove previous NGINX installation"
...
echo_task 14/40 "Create NGINX conf.d directory"
mkdir -p /etc/nginx/conf.d
check_exit_code 14

echo_task 15/40 "Download blacklist and block IPs for NGINX"
...
check_exit_code 15.1

echo_task 16/40 "Create default site configuration for NGINX"
...
echo_task 17/40 "Create fastcgi.conf for NGINX"
...
echo_task 18/40 "Create fastcgi-php.conf for NGINX"
...
check_exit_code 22.1

echo_task 23/40 "Update package list for MariaDB"
apt update -y -q
check_exit_code 23

echo_task 24/40 "Configure and install MariaDB"
...
echo_task 25/40 "Add custom configuration for MySQL"
...
echo_task 26/40 "Write down current password for MariaDB in my.cnf"
echo -e "\n[client]\nuser = root\npassword = $password" >> /etc/mysql/my.cnf
check_exit_code 26

echo_task 27/40 "Restart MariaDB"
service mysql restart
check_exit_code 27

echo_task 28/40 "Install MySQLTuner"
apt install -y -q mysqltuner
check_exit_code 28

echo_task 29/40 "Create default folder for future websites"
mkdir -p /var/www/test.com
check_exit_code 29

echo_task 30/40 "Create Hello World page"
echo -e "<html>\n<body>\n<h1>Hello World!<h1>\n</body>\n</html>" > /var/www/test.com/index.html
check_exit_code 30

echo_task 31/40 "Create opcache page"
wget -O /var/www/test.com/opcache.php https://raw.githubusercontent.com/rlerdorf/opcache-status/master/opcache.php
check_exit_code 31

echo_task 32/40 "Create phpinfo page"
echo -e "<?php phpinfo();" > /var/www/test.com/info.php
check_exit_code 32

echo_task 33/40 "Give NGINX permissions to access websites"
chown -R www-data:www-data /var/www/*
check_exit_code 33

echo_task 34/40 "Maximize the limits of file system usage"
echo -e "*       soft    nofile  1000000" >> /etc/security/limits.conf
echo -e "*       hard    nofile  1000000" >> /etc/security/limits.conf
check_exit_code 34

echo_task 35/40 "Switch to ondemand state of PHP-FPM"
sed -i "s/^pm = .*/pm = ondemand/" /etc/php/8.3/fpm/pool.d/www.conf
check_exit_code 35

echo_task 36/40 "Calculate and set number of children for PHP-FPM"
...
echo_task 38/40 "Reload Nginx and PHP-FPM"
