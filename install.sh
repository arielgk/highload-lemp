...
echo_task 15/40 "Disable external access to PHP-FPM scripts"
sed -i "s/^;cgi.fix_pathinfo=1/cgi.fix_pathinfo=0/" /etc/php/8.3/fpm/php.ini
check_exit_code 15

echo_task 16/40 "Create NGINX conf.d directory"
mkdir -p /etc/nginx/conf.d
check_exit_code 16

echo_task 17/40 "Download blacklist and block IPs for NGINX"
...
check_exit_code 17.1

echo_task 18/40 "Create default site configuration for NGINX"
...
echo_task 19/40 "Create fastcgi.conf for NGINX"
...
echo_task 20/40 "Create fastcgi-php.conf for NGINX"
...
check_exit_code 22.2

echo_task 23/40 "Restart memcached service"
service memcached restart
check_exit_code 23

echo_task 24/40 "Add repository for MariaDB 10.6"
...
check_exit_code 24.1

echo_task 25/40 "Update package list for MariaDB"
apt update -y -q
check_exit_code 25

echo_task 26/40 "Configure and install MariaDB"
...
echo_task 27/40 "Add custom configuration for MySQL"
...
echo_task 28/40 "Write down current password for MariaDB in my.cnf"
echo -e "\n[client]\nuser = root\npassword = $password" >> /etc/mysql/my.cnf
check_exit_code 28

echo_task 29/40 "Restart MariaDB"
service mysql restart
check_exit_code 29

echo_task 30/40 "Install MySQLTuner"
apt install -y -q mysqltuner
check_exit_code 30

echo_task 31/40 "Create default folder for future websites"
mkdir -p /var/www/test.com
check_exit_code 31

echo_task 32/40 "Create Hello World page"
echo -e "<html>\n<body>\n<h1>Hello World!<h1>\n</body>\n</html>" > /var/www/test.com/index.html
check_exit_code 32

echo_task 33/40 "Create opcache page"
wget -O /var/www/test.com/opcache.php https://raw.githubusercontent.com/rlerdorf/opcache-status/master/opcache.php
check_exit_code 33

echo_task 34/40 "Create phpinfo page"
echo -e "<?php phpinfo();" > /var/www/test.com/info.php
check_exit_code 34

echo_task 35/40 "Give NGINX permissions to access websites"
chown -R www-data:www-data /var/www/*
check_exit_code 35

echo_task 36/40 "Maximize the limits of file system usage"
echo -e "*       soft    nofile  1000000" >> /etc/security/limits.conf
echo -e "*       hard    nofile  1000000" >> /etc/security/limits.conf
check_exit_code 36

echo_task 37/40 "Switch to ondemand state of PHP-FPM"
sed -i "s/^pm = .*/pm = ondemand/" /etc/php/8.3/fpm/pool.d/www.conf
check_exit_code 37

echo_task 38/40 "Calculate and set number of children for PHP-FPM"
...
echo_task 39/40 "Enable PHP-FPM Opcache"
...
echo_task 40/40 "Compile NGINX Brotli and re-install NGINX"
rm -rf /tmp/build
mkdir -p /tmp/build
cd /tmp/build
